...
class MPCInitializerNode(Node):
    ...
    def __init__(self):
        ...
        config = {
            "mpc": {
                "Q_rows": [0, 1],
                "Qz": 100.0, # was circles: 500, 1000, eight: 700
                "Qzf": 0,  # was 640
                "R": 0.0,
                "Rdu": 0.1,
                "U_constraint": 0.9,
            ...
            "trajectory": {
                "type": "controlled_csv",  # Options: "circle", "circle_with_ramp", "eight", "pacman", "pacman_with_ramp", "flower", "controlled_csv"
                "duration": 10.0,  # Duration of the simulation in seconds
                "speed": 0.5,  # Angular speed (rad/s)
                "include_velocity": False,
                "csv_path": os.path.join(self.data_dir, 'trajectories/dynamic/observations_controlled_305.csv'),
                "parameters": {
                    "center": [0.10730, 0.12463],  # Center of the (x,y) trajectory
                    "radius": 0.05,  # [m]  For "circle" and "pacman"
                    "amplitude": 0.05,  # [m]  For "eight"
                    "z_level": -0.40798,  # [m]  Constant z-coordinate
        ...
        print(self.ref_traj.trajectory.shape)
        ...
        Qzf = 0.0*jnp.eye(3)
        R = 0*jnp.eye(6)
        R_du_diag = jnp.array([1, 0.1, 1, 0.1, 1, 1])*10
