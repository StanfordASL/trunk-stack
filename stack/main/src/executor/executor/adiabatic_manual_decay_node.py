...
from interfaces.msg import TrunkMarkers, TrunkRigidBodies, AllMotorsStatus


class DataCollectorNode(Node):
    def __init__(self):
        super().__init__('adiabatic_manual_decay_node')
        self.declare_parameters(namespace='', parameters=[
            ...
            ('collect_angles', True)            # whether or not to collect motor angle data
        ])
    ...
    def process_data(self, names):
        ...
        with open(trajectory_csv_file, 'a', newline='') as file:
            ...
            for i, pos_list in enumerate(self.stored_positions):
                angle_list =self.stored_angles[i]
                row = [self.trajectory_count] + [coord for pos in pos_list for coord in [pos.x, pos.y, pos.z]] + [angle for angle in angle_list]
                writer.writerow(row)
