...
class GuSTO:
    ...
    #     t_dyn_jac = time.time() - t_jac_start
    ...
    #     t_perf_jac = time.time() - t_perf_start

    #     print(f'First Jacobian timing:')
    #     print(f' Dynamics linearization: {t_dyn_jac*1000:.2f} ms')
    #     print(f' Performance linearization: {t_perf_jac*1000:.2f} ms')
    ...
    #                 print(f'Iteration {itr} Jacobian timing:')
    ...
    #     self.xopt = jnp.copy(self.x_k)
    #     self.uopt = jnp.copy(self.u_k)
    #     #print(self.xopt.shape, self.uopt.shape)
    #     if self.nonlinear_perf_mapping:
    ...
    # def solve(self, x0, u_init, x_init, z=None, zf=None, u=None):
    #     """
    #     Solve the GuSTO problem - accurate timing without accumulation bugs.
    #     """
    #     import time
    ...
    #     print(f'First Jacobian timing:')
    #     print(f' Dynamics linearization: {t_first_dyn*1000:.2f} ms')
    #     print(f' Performance linearization: {t_first_perf*1000:.2f} ms')
        
    #     if self.verbose >= 1:
    #         print('|   J   | TR_viol |  rho_k  |  X_viol |   x-x_k |  delta  |  omega |')
    #         print('--------------------------------------------------------------------')
        
    #     t_locp_update_total = 0.0
    #     t_locp_solve_total = 0.0
    #     t_iter_dyn_total = 0.0
    #     t_iter_perf_total = 0.0
    ...
    #         else:
    #             self.locp.update(A_d, B_d, d_d, x0, self.x_k, delta, omega, z=z, zf=zf, u=u, Hd=H_d, Gd=G_d, cd=c_d, full=False)
    #         t_locp_update_total += time.time() - t0
            
    #         # LOCP Solve
    #         t0 = time.time()
    #         Jstar, success, stats = self.locp.solve()
    #         t_locp_solve_total += time.time() - t0
            
    #         if not success:
    #             print(f'Iteration {itr} of problem cannot be solved')
    #             self.xopt = jnp.copy(self.x_k)
    #             self.uopt = jnp.copy(self.u_k)
    #             if self.nonlinear_perf_mapping:
    #                 self.zopt = self.performance_mapping(self.xopt.T, self.uopt.T).T
    ...
    #             if self.max_gusto_iters >= 1:
    #                 # Time ONLY this iteration's Jacobians (not accumulated!)
    #                 t0 = time.time()
    #                 A_d, B_d, d_d = self._get_dynamics_linearizations(self.x_k, self.u_k)
    #                 t_iter_dyn = time.time() - t0
    #                 t_iter_dyn_total += t_iter_dyn
    ...
    #                 else:
    #                     H_d, G_d, c_d = None, None, None
    #                 t_iter_perf = time.time() - t0
    #                 t_iter_perf_total += t_iter_perf
                    
    #                 print(f'Iteration {itr} Jacobian timing:')
    #                 print(f'  Dynamics: {t_iter_dyn*1000:.2f} ms, Performance: {t_iter_perf*1000:.2f} ms')  # ← Single call only!
    ...
    #     else:
    #         print(f'\nTiming breakdown:')
    #         print(f'  First Jacobians: {(t_first_dyn + t_first_perf)*1000:.2f} ms')
    #         print(f'  Iter Jacobians: {(t_iter_dyn_total + t_iter_perf_total)*1000:.2f} ms')
    #         print(f'  LOCP update: {t_locp_update_total*1000:.2f} ms')
    #         print(f'  LOCP solve: {t_locp_solve_total*1000:.2f} ms')
    #         print(f'  Other: {(t_total - t_first_dyn - t_first_perf - t_iter_dyn_total - t_iter_perf_total - t_locp_update_total - t_locp_solve_total)*1000:.2f} ms')
    #         print(f'Solved in {itr} iterations/{t_total:.3f} seconds, with {t_locp_solve_total:.3f} s from LOCP solve')
    ...
    #     else:
    #         self.zopt = jnp.transpose(self.H @ self.xopt.T)
    #     self.locp_solve_time = t_locp_solve_total
    ...
    # def solve(self, x0, u_init, x_init, z=None, zf=None, u=None):
    #     """
    #     Solve with COMPLETE timing breakdown. important
    #     """
    #     import time
    ...
    #                 self.zopt = jnp.transpose(self.H @ self.xopt.T)
    #             return
    #         t_copy_1 +=time.time()-t0

    #         # Get solution
    #         t0 = time.time()
    #         x_next, u_next, _ = self.locp.get_solution()
    #         t_locp_get_sol += time.time() - t0
            
    #         # Trust region check
    #         t0 = time.time()
    #         e_tr, tr_satisfied = self._is_in_trust_region(self.x_k, x_next, delta)
    #         t_trust_region += time.time() - t0
            
    #         if tr_satisfied:
    #             # Accuracy computation
    #             t0 = time.time()
    #             rho_k = self._compute_accuracy(self.x_k, self.u_k, x_next, u_next, Jstar)
    #             t_accuracy += time.time() - t0
    ...
    #                 print(f'Iteration {itr} Jacobian timing:')
    #                 print(f'  Dynamics+Perf: {t_iter_jac*1000:.2f} ms')
        
    #     t_total = time.time() - t_total_start
        
    #     # Calculate what's left
    #     t_accounted = (t_first_dyn + t_first_perf + t_iter_jac + t_locp_update + 
    #                 t_locp_solve + t_locp_get_sol + t_trust_region + t_accuracy + 
    #                 t_constraints + t_convergence + t_copy + t_copy_1)
    #     t_mystery = t_total - t_accounted
    ...
    #         print(f'')
    #         print(f'Jacobians:')
    #         print(f'  First: {(t_first_dyn + t_first_perf)*1000:.2f} ms')
    #         print(f'  Iterations: {t_iter_jac*1000:.2f} ms')
    #         print(f'')
    #         print(f'LOCP:')
    #         print(f'  Update: {t_locp_update*1000:.2f} ms')
    #         print(f'  Solve: {t_locp_solve*1000:.2f} ms')
    #         print(f'  Get solution: {t_locp_get_sol*1000:.2f} ms')
    #         print(f'')
    #         print(f'Checks:')
    #         print(f'  Trust region: {t_trust_region*1000:.2f} ms')
    #         print(f'  Accuracy: {t_accuracy*1000:.2f} ms')
    #         print(f'  Constraints: {t_constraints*1000:.2f} ms')
    #         print(f'  Convergence: {t_convergence*1000:.2f} ms')
    #         print(f'')
    #         print(f'Other:')
    #         print(f'  Copy ops: {t_copy*1000:.2f} ms')
    #         print(f'  Mystery: {t_mystery*1000:.2f} ms')
    #         print(f'  Copy 1: {t_copy_1*1000:.2f} ms')
    #         print(f'=' * 40)
    #         print(f'Solved in {itr} iterations/{t_total:.3f} seconds')
    ...
    # def solve(self, x0, u_init, x_init, z=None, zf=None, u=None):
    #     """
    #     Solve the GuSTO problem with detailed timing breakdowns.
    #     """
    #     import time
    ...
    #         'locp_update': 0.0,
    #         'locp_solve': 0.0,
    #         'locp_get_solution': 0.0,
    #         'trust_region_check': 0.0,
    #         'accuracy_compute': 0.0,
    ...
    #     A_d, B_d, d_d = self._get_dynamics_linearizations(self.x_k, self.u_k)
    #     # Force completion
    #     if hasattr(A_d, 'block_until_ready'):
    #         A_d[0].block_until_ready()

    #     timings['first_dyn_jac'] = time.time() - t0
        
    #     t0 = time.time()
    #     if self.nonlinear_perf_mapping:
    #         H_d, G_d, c_d = self._get_perf_mapping_linearizations(self.x_k, self.u_k)
    #         if hasattr(H_d, 'block_until_ready'):
    #             H_d[0].block_until_ready()
    #     else:
    #         H_d, G_d, c_d = None, None, None
    #     timings['first_perf_jac'] = time.time() - t0
        
    #     print(f'First Jacobian timing:')
    #     print(f' Dynamics linearization: {timings["first_dyn_jac"]*1000:.2f} ms')
    #     print(f' Performance linearization: {timings["first_perf_jac"]*1000:.2f} ms')
    ...
    #         # Force any JAX operations in update to complete
    #         jax.block_until_ready(self.x_k)    
    #         timings['locp_update'] += time.time() - t0
            
    #         # LOCP Solve
    #         t0 = time.time()
    #         Jstar, success, stats = self.locp.solve()
    #         # Force solve to complete
    #         jax.block_until_ready(Jstar)
    #         timings['locp_solve'] += time.time() - t0
            
    #         if not success:
    #             print(f'Iteration {itr} of problem cannot be solved, see solver status for more information')
    ...
    #             else:
    #                 self.zopt = jnp.transpose(self.H @ self.xopt.T)
    #             timings['copy_operations'] += time.time() - t0
    #             return
    ...
    #         timings['locp_get_solution'] += time.time() - t0
            
    #         # Trust region check
    #         t0 = time.time()
    #         e_tr, tr_satisfied = self._is_in_trust_region(self.x_k, x_next, delta)
    #         jax.block_until_ready(e_tr)
    #         timings['trust_region_check'] += time.time() - t0
    ...
    #             t0 = time.time()
    #             rho_k = self._compute_accuracy(self.x_k, self.u_k, x_next, u_next, Jstar)
    #             jax.block_until_ready(rho_k)
    #             timings['accuracy_compute'] += time.time() - t0
    ...
    #                 t0 = time.time()
    #                 max_violation, X_satisfied = self._state_constraints_violated(x_next)
    #                 jax.block_until_ready(max_violation)
    #                 timings['constraint_check'] += time.time() - t0
    ...
    #         timings['logic_overhead'] += time.time() - t_logic
    ...
    #                 if hasattr(A_d, 'block_until_ready'):
    #                     A_d[0].block_until_ready()
    #                 timings['iter_dyn_jac'] += time.time() - t0
    ...
    #                 else:
    #                     H_d, G_d, c_d = None, None, None
    #                 timings['iter_perf_jac'] += time.time() - t0
                    
    #                 print(f'Iteration {itr} Jacobian timing:')
    #                 print(f'  Dynamics: {timings["iter_dyn_jac"]*1000:.2f} ms, Performance: {timings["iter_perf_jac"]*1000:.2f} ms')
    #     # Force everything to finish before final timing
    #     jax.block_until_ready(self.x_k)
    #     t_total = time.time() - t_total_start
        
    #     # Calculate misc time (unaccounted)
    #     accounted = sum(timings.values())
    #     timings['misc'] = t_total - accounted
        
    #     # Print detailed timing breakdown
    #     print('\n=== DETAILED TIMING BREAKDOWN ===')
    #     print(f'Total time: {t_total*1000:.2f} ms')
    #     print(f'Accounted time: {accounted*1000:.2f} ms')
    #     print(f'Unaccounted (misc): {timings["misc"]*1000:.2f} ms')
    #     print('\nBreakdown by component:')
    #     for key, val in sorted(timings.items(), key=lambda x: -x[1]):
    #         if val > 0.0001:  # Only show non-zero timings > 0.1ms
    #             pct = (val / t_total) * 100
    #             print(f'  {key:25s}: {val*1000:7.2f} ms ({pct:5.1f}%)')
    #     print('=' * 50)
    ...
    #         print('Max iterations, solution did not converge')
    #     else:
    #         print(f'Solved in {itr} iterations/{t_total:.3f} seconds, with {timings["locp_solve"]:.3f} s from LOCP solve')
    ...
    #     self.locp_solve_time = timings['locp_solve']
